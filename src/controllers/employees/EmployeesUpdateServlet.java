...
public class EmployeesUpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(_token != null && _token.equals(request.getSession().getId())) {
            ...
            if(e.getCode().equals(request.getParameter("code"))) {//もし変更されていなければ、以下の変数にfalse代入
                code_duplicate_check = false;
            } else {//違うのならば（変更したのならば）今回入力した値をセット→重複チェック
                e.setCode(request.getParameter("code"));
            }
            /*入力されたパスワードの入力値チェック→もし入力されていれば、else以下を実行*/
            ...
            /*（社員番号・パスワード以外で）フォームに入力された値のセット*/
            e.setName(request.getParameter("name"));
            e.setAdmin_flag(Integer.parseInt(request.getParameter("admin_flag")));
            e.setUpdated_at(new Timestamp(System.currentTimeMillis()));
            e.setDelete_flag(0);

            /*上のメソッドで入手した二つの変数の値（重複＆パスワードチェック）を引数としてバリデーション*/
